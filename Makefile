...
install-utilities:	install-mkgroup build-utilities
	...
	[ ! -f /home/`whoami`/econetfs/SYSTEM/PIFSTOOL -a ! -L /home/`whoami`/econetfs/SYSTEM/PIFSTOOL ] && cp FS/PIFSTOOL /home/`whoami`/econetfs/0PIBRIDGE-00/SYSTEM/PIFSTOOL 
	[ ! -f /etc/econet-gpio/pserv.sh -a ! -L /etc/econet-gpio/pserv.sh ] && sudo cp config/pserv.sh /etc/econet-gpio
	...
	[ ! -f /etc/econet-gpio/econet-hpbridge.cfg && ! -L /etc/econet-gpio/econet-hpbridge.cfg ] && (sudo cp config/econet-hpbridge.cfg-EconetPlusFileserver.local /etc/econet-gpio/econet-hpbridge.cfg ; sudo chown `whoami` /etc/econet-gpio/econet-hpbridge.cfg )
	[ ! -f /etc/systemd/system/econet-hpbridge.service && ! -L /etc/systemd/system/econet-hpbridge.service ] && sudo cp systemd/econet-hpbridge.service.local /etc/systemd/system/econet-hpbridge.service
