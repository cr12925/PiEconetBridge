...
/* 20251127 Some new structs - see if we can make rx / tx IRQs more efficient by having some things pre-built, and moving the memcpy()s to userspace */

/* Pre-built scout, ack ,etc. for speed */
...
struct  econet_scout {
                ...
                u8      ctrl;
                u8      port;
                u8      data[ECONET_MAX_PACKET_SIZE]; /* Some scouts have data on them, and we can put broadcasts and 2-way immediates in here too */
        };

struct econet_scout econet_tx_scout;
struct econet_scout econet_rx_scout;
...
struct econet_ack {
                ...
                u8      srcstn;
                u8      srcnet;
                u8      data[10]; /* In case we get a scout instead of an ack - which sometimes happens  - 10 bytes for 8 data + port + ctrl just in case */
        };

struct econet_ack econet_rx_ack;
struct econet_ack econet_tx_ack;

struct econet_4way_data {
                u8      dststn;
                u8      dstnet;
                u8      srcstn;
                u8      srcnet;
                u8      data[ECONET_MAX_PACKET_SIZE];
        };

struct econet_4way_data econet_4way_data_rx; /* Shouldn't really need this except in raw mode */
struct econet_4way_data econet_4way_data_tx;

struct econet_packet {
        struct econet_scout     *scout;
        struct econet_4way_data *data; /* Will be NULL for broadcasts and 2-way immediates */
        u8                      aun_ttype;
        u16                     scout_data; /* Number of bytes on scout */
        u16                     data_len; /* Number of bytes of data in the 4-way section; NB raw mode always just receives to 4way struct */
        u8                      sr2; /* Contents of sr2 when the frame ended, is 0 for transmissions */
};
