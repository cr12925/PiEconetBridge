...
uint8_t econet_remote_poll(int ms)
{
	...
	if ((p.revents & POLLIN)) // Econet data arrived
	{
		...
		if (len > 12 && r.p.port == 0xB1)
		{
			...
			for (int count = 0; count < r.p.data[11]; count++)
				fprintf (stderr, "%c", r.p.data[12+count]);
...
void econet_pipeeg_run(void)
{
	...
	// Now send FindServer broadcast

	p.p.dststn = p.p.dstnet = 0xff;
	p.p.aun_ttype = ECONET_AUN_BCAST;
	p.p.port = 0xB0;
	p.p.ctrl = 0x80;
	memset (&(p.p.data), 0x20, 8);
	...
	fprintf (stderr, "Net.Stn  Ver  Type     Name\n");
