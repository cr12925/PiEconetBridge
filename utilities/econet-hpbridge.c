...
// Encrypted version stops here
							...
							if ((p->tx)++ < EB_CONFIG_WIRE_RETRIES)
							{
								...
								else if (result == p->length + 12) // Only if we wrote it all correctly!
								{
									...
									if (err == ECONET_TX_SUCCESS)
									{
										...
										if (tx.p.aun_ttype == ECONET_AUN_DATA && p->length > 500 && pt.data_end > pt.data_start) /* Attempt to discern clock rate from time taken to transmit 3rd part of 4-way handshake (the data portion, which will usually be the longest) - but we don't bother if it's a short-ish packet (<= 500 bytes) */
										{
											uint64_t	data_tx_time;

											data_tx_time = pt.data_end - pt.data_start;

											data_tx_time = (1000000000 / data_tx_time) * (p->length * 8); /* Calculates how many bits would have been transmitted in 1 second */

											data_tx_time /= 1000; /* Convert to kb/s */
					...
					case EB_DEF_LOCAL:
					{
						...
						else if (p->p->p.port == 0x00 && p->p->p.ctrl == 0x88 && p->p->p.aun_ttype == ECONET_AUN_IMM)
						{
							...
							ack.p.data[1] = eb_mfr; ack.p.data[0] = eb_mtype;
							ack.p.data[2] = (EB_VERSION); // & 0xff);
							ack.p.data[3] = 5; // (EB_VERSION & 0xff00) >> 8;
...
int main (int argc, char **argv)
{
	...
	*/
	...
	EB_CONFIG_WIRE_RETRIES = 30; /* 20250802 Increased from 10; and 20260425 to 30 - Beebs sometimes aren't listening! */
	EB_CONFIG_WIRE_MAX_NOTLISTENING = 10; // Number of not listenings to treat as non-fatal on transmission of a 4-way (to cope with RISC OS bug where it sometimes isn't listening for a data burst on getbytes/load  20260425 increased to 10
