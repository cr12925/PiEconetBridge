...
int fsop_cdir_internal(struct fsop_data *f, unsigned char *path, uint8_t relative_to)
{
	...
	if (!fsop_normalize_path(f, path, relative_to, &p))
	       return -3;
...
void fsop_do_cdir(struct fsop_data *f, unsigned char *path, uint8_t relative_to)
{
	...
	e = fsop_cdir_internal(f,path,relative_to);
	...
	else
	{
		switch (e * -1)
		{
			...
			case 3: fsop_error(f, 0xFF, "Not found"); break;
			case 5: fsop_error(f, 0xFF, "Insufficient access"); break;
...
FSOP(1b)
{
	...
	fs_debug_full (0, 1, f->server, f->net, f->stn, "*CDIR %s", path);
