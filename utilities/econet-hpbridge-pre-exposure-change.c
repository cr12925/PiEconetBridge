	...
	if (p->p.port == 0x9C) // Bridge traffic
	{
		...
		else if ((p->p.ctrl & 0xFE) == 0x80) // Incoming reset or update
		{
			...
			while (data_count < length)
			{
				...
				snprintf (net_string, 5, " %d", in_adv);
...
static void * eb_device_listener (void * device)
{
	...
	while (poll(&p, 1, -1))
	{
		if ((d->pipe.skt_write != -1) && (p.revents & POLLHUP)) // Presumably PIPE - close writer socket
		{
...
/* Disabled during implementation of inactive listeners

	if (!e->parent) // Die - no parent network
		return NULL;

	if (e->parent->type == EB_DEF_AUN) // Refuse to start. We can't expose remote AUN to AUN!
		eb_debug (1, 0, "CONFIG", "%-8s %3d.%3d Attempt to expose distant AUN to local AUN listener. Forbidden.",
			eb_type_str(e->parent->type), e->net, e->stn);
*/
	...
	if (e->socket == -1)
		eb_debug (1, 0, "LISTEN", "%-8s     Unable to open AUN listener socket for station %d.%d on port %d", "AUN", e->net, e->stn, e->port);
		...
		else if (eb_is_exposure_active(e)) // Dump packet if exposure inactive
		{
			...
			if (!source_device) // See if we can allocate a dynamic host
			{
				...
				if (found)
				{
					...
					eb_debug (0, 2, "DYNAMIC", "%-8s %3d.%3d Traffic received from unknown source %s:%d - Allocated dynamic host.", eb_type_str(source_device->type), station->eb_device->net, station->stn, inet_ntoa(addr.sin_addr), source_port);
...
static void * eb_device_despatcher (void * device)
{
	...
	switch (d->type)
	{
		...
		case EB_DEF_TRUNK:
		{
			...
			//eb_debug (0, 2, "DESPATCH", "%-8s     Trunk listener socket opened", "Trunk");
	...
	while (1)
	{
		...
		if (!(d->type == EB_DEF_WIRE && wire_null) && (d->type != EB_DEF_LOCAL || (d->local.ip.tunif[0] != '\0'))) while (poll(&p, 1, 0) && (p.revents & POLLIN)) // A 0-time poll() apparently works
		{
			...
			if (d->type == EB_DEF_WIRE || d->type == EB_DEF_TRUNK) // Read straight to packet structure
			{
			...
			if (length >= 12)	 // Should always have at the least 4 bytes of addressing and 8 bytes AUN
			{
