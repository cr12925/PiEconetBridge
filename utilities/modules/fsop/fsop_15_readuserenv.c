...
FSOP(15)
{
	...
	fs_debug_full (0, 2, f->server, f->net, f->stn, "Read user environment - current user handle %d (dir:%c ptr:%p), current lib handle %d (dir:%c ptr:%p)", a->current, a->fhandles[a->current].is_dir ? 'Y' : 'N',
				a->fhandles[a->current].handle, a->lib, a->fhandles[a->lib].is_dir ? 'Y' : 'N', a->fhandles[a->lib].handle);
