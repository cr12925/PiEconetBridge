...
void fsop_port99 (struct __fs_station *s, struct __econet_packet_aun *packet, uint16_t datalen)
{
	...
	fsop_param.net = packet->p.srcnet; /* Translater sender net to 0 if local */
...
void fsop_setup(void)
{
	...
	FSOP_SET (2d, (FSOP_F_LOGGEDIN)); /* 32 bit Examine */
	...
	FSOP_OSCLI(CDIR,(FSOP_00_LOGGEDIN),1,1,2);
	FSOP_OSCLI(CHOWN,(FSOP_00_ANON), 1, 2, 5);
	FSOP_OSCLI(COPY,(FSOP_00_LOGGEDIN), 1, 2, 3);
	FSOP_OSCLI(CREDIT,(FSOP_00_LOGGEDIN | FSOP_00_MDFS | FSOP_00_SYSTEM), 2, 2, 3);
	...
	FSOP_OSCLI(FSDCMD,(FSOP_00_LOGGEDIN | FSOP_00_SYSTEM), 2, 10, 4);
	...
	FSOP_OSCLI(PRINTOUT,(FSOP_00_LOGGEDIN | FSOP_00_MDFS), 1, 1, 6);
